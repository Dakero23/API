...
import co.com.api.prueba.classabstract.AbstractTarjetaDao;
...
public class tarjetaApp {
    ...
    public Response actualizarTarjeta(Tarjeta tarjeta) {
        ...
        return Response.status(Response.Status.ACCEPTED).entity(AbstractTarjetaDao.actualizaTarjeta(tarjeta)).build();
    }
    ...
    public Response consultarUsuario(@QueryParam("id") int id) {
        ...
        if (id > 0) {
            Tarjeta p_tarjeta = new Tarjeta();
            p_tarjeta.setTarId(id);
            query = AbstractTarjetaDao.query_pk_Tarjeta(p_tarjeta);

        } else {
            query = AbstractTarjetaDao.query_todos_Tarjeta();
        }
        if (query.getEstadoProceso().equals("N")) {
            return Response.status(406).entity(query).build();
        }
        List<Tarjeta> usuario = (List<Tarjeta>) query.getObjetoRetorno();
        Gson jsonConverter = new GsonBuilder().create();
        return Response.status(Response.Status.ACCEPTED).entity(jsonConverter.toJson(usuario)).build();
    }

    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response crearUsuario(Tarjeta p_tarjeta) {
        GestionRetornoObjeto obj = AbstractValidacion.validarToken(headers);
        if (obj.getEstadoProceso().equals("N")) {
            return Response.status(406).entity(obj).build();
        }
        GestionRetornoObjeto query = AbstractTarjetaDao.insertaTarjeta(p_tarjeta);
        Gson gson = new Gson();
    ...
    public GestionRetornoObjeto eliminarUsuario(@QueryParam("id") int id) {
        Tarjeta p_tarjeta = new Tarjeta();
        p_tarjeta.setUsuId(id);
        return AbstractTarjetaDao.eliminaTarjeta(p_tarjeta);
    }
